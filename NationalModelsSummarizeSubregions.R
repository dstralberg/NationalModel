...
w <-"I:/My Drive/BAM.SharedDrive/RshProjs/PopnStatus/NationalModels/feb2020/artifacts/"
...
for (j in 3:length(specpred)) {
  species<-specpred[j]
  d <- sumdens(species,landcov,units)
  write.csv(d,file=paste0(x,"/summaries/",species,"_densities_subunit.csv"),row.names=FALSE)
}
