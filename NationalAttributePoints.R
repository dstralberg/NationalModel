...
fw750<-focalWeight(x=bs2011,d=750,type="Gauss") #Gaussian filter with sigma=750 (tapers off around 2km)
...
# bs2001_Gauss750<-brick(focal(bs2001[[1]],w=fw750,na.rm=TRUE))
# names(bs2001_Gauss750)<-names(bs2001)[[1]]
# for(i in 2:nlayers(bs2001)){
#   bs2001_Gauss750<-addLayer(bs2001_Gauss750,focal(bs2001[[i]],w=fw750,na.rm=TRUE))
#   names(bs2001_Gauss750)[i]<-names(bs2001)[[i]]
# }
# writeRaster(bs2001_Gauss750,file=paste(w,"bs2001_750",sep=""))
bs2001_Gauss750 <- brick(paste(w,"bs2001_750.grd",sep=""))
ss <- cbind(ss,extract(bs2001_Gauss750,st_coordinates(ss)))
