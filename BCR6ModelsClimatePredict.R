...
brtplot <- function (j) {
  load(paste(w1,speclist[j],"brt7.R",sep=""))
  ...
  write.csv(cvstats,file=paste(w1,speclist[j],"cvstats7.csv",sep=""))
  ...
  max <- 3*prev
  png(file=paste(w1,speclist[j],"_pred1km7.png",sep=""), height=800, width=650)
  ...
  png(file=paste(w,speclist[j],"_pred1km7_pts.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
...
mapplot <- function (j) {
  rast <- raster(paste(w1,speclist[j],"_pred1km7.tif",sep=""))
  prev <- cellStats(rast, 'mean')	
  max <- 3*prev
  png(file=paste(w1,speclist[j],"_pred1km7.png",sep=""), height=800, width=650)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  ...
  xy <- PC1[,c(7,8)]
  spdf <- SpatialPointsDataFrame(coords = xy, data = PC1, proj4string = LCC)
  png(file=paste(w1,speclist[j],"_pred1km7_pts.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
  ...
  xy <- PC1[,c(7,8)]
  spdf <- SpatialPointsDataFrame(coords = xy, data = PC1, proj4string = LCC)
  png(file=paste(w1,speclist[j],"_pred1km7_pts_small.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
...
cvstatsum <- function (speclist) {
  cvstats <- read.csv(paste(w1,speclist[1],"cvstats7.csv",sep=""))
  ...
  for (j in 2:length(speclist)) {
    x<-try(cv2 <- read.csv(paste(w1,speclist[j],"cvstats7.csv",sep="")))
...
for (j in 1:length(speclist)) {
  ...
  if (class(x1) != "NULL") {
    save(brt1,file=paste(w1,speclist[j],"brt7.R",sep=""))
    brtplot(j)
  }
...
for (j in 1:length(speclist)) {
  x1 <- try(load(paste(w1,speclist[j],"brt7.R",sep="")))
...
write.csv(cvstats,file=paste(w1,"_cvstats7.csv",sep=""))
