...
for (j in 1:length(speclist)) {
  ...
  pred1 <- na.omit(pred1)
  #t <- glm(obs ~ YEAR*bcrsu, data=pred1, weights=wi, offset=offset+log(pred))
  # pred1$lresid <- log(pred1$resid)
  t <- lm(log(resid+0.5) ~ YEAR*bcrsu, data=pred1, weights=wi)
  ...
  write.csv(beta,file=paste(speclist[j],"_betas_subregion_adjust_long.csv",sep=""), row.names=FALSE)
  # modelout1 <- tidy(t)
  # modelout1$species <- speclist[j]
  # modelout1$su <- pred1$bcrsu[1]
  # modelout <- rbind(modelout,modelout1)
  # beta1 <- as.data.frame(t$coefficients)
  # names(beta1) <- as.character(speclist[j])
  # beta <- cbind(beta,beta1)
  }
}
# write.csv(modelout,file=paste("_modelout_subregion.csv",sep=""))
# betayr <- mefa:::rep.data.frame(beta[2,], 15)

# beta2 <- rbind(beta[18:32,] + betayr)
# write.csv(beta2,file="_betas_subregion_adjust.csv")
# beta2$su <- row.names(beta2)
# betalong <- gather(beta2, species, trend, ALFL:YRWA, factor_key=TRUE)

yr <- 0:20

for (j in 1:length(speclist)) {
    x<-try(b <- read.csv(paste(speclist[j],"_betas_subregion_adjust_long.csv",sep="")))
    if (class(x) != "try-error") {
    names(b) <- c("trend","se","species","su")
    b$su <- gsub("bcrsu","",b$s)uborea
    ...
    pdf(paste(speclist[j],"_trends.pdf",sep="")) 
