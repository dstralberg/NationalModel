...
bs<-stack(paste(w,"BCR6/bcr6clim_1km.grd",sep=""))
bs2 <- stack("G:/Boreal/NationalModelsV2/BCR6/bcr6_1km.grd")
r2 <- bs2[[1]]
...
#write.csv(offcombo,"G:/Boreal/NationalModelsV2/BCR6/offcombo.csv",row.names=FALSE)
...
# off6 <- inner_join(offcombo,survey,by="PKEY")[,1:5]
# write.csv(off6,"G:/Boreal/NationalModelsV2/BCR6/off6.csv",row.names=FALSE)
off6 <- read.csv("G:/Boreal/NationalModelsV2/BCR6/off6.csv")
...
brtplot <- function (j) {
  load(paste(w1,speclist[j],"brt6.R",sep=""))
  ...
  write.csv(cvstats,file=paste(w1,speclist[j],"cvstats6.csv",sep=""))
  pdf(paste(w1,speclist[j],"_plot6.pdf",sep=""))
  gbm.plot(brt1,n.plots=12,smooth=TRUE)
  dev.off()
  rast <- raster::predict(bs2, brt1, type="response", n.trees=brt1$n.trees)
  writeRaster(rast, filename=paste(w1,speclist[j],"_pred1km6",sep=""), format="GTiff",overwrite=TRUE)
  
  prev <- cellStats(rast, 'mean')	
  max <- 3*prev
  png(file=paste(w1,speclist[j],"_pred1km6.png",sep=""), height=800, width=650)
  ...
  png(file=paste(w,speclist[j],"_pred1km6_pts.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
  ...
  png(file=paste(w,speclist[j],"_pred1km6_pts_small.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
...
mapplot <- function (j) {
  rast <- raster(paste(w1,speclist[j],"_pred1km6.tif",sep=""))
  prev <- cellStats(rast, 'mean')	
  max <- 3*prev
  png(file=paste(w1,speclist[j],"_pred1km6.png",sep=""), height=800, width=650)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  ...
  xy <- PC1[,c(7,8)]
  spdf <- SpatialPointsDataFrame(coords = xy, data = PC1, proj4string = LCC)
  png(file=paste(w1,speclist[j],"_pred1km6_pts.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
  ...
  png(file=paste(w1,speclist[j],"_pred1km6_pts_small.png",sep=""), width=650, height=800)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
  plot(rast, col="blue", axes=FALSE, legend=FALSE, main=paste(as.character(speclist[j]),"current prediction"))
  plot(rast, col=bluegreen.colors(15), zlim=c(0,max), axes=FALSE, main=as.character(speclist[j]), add=TRUE, legend.width=1.5, horizontal = TRUE, smallplot = c(0.60,0.85,0.84,0.87), axis.args=list(cex.axis=1.2))
...
futplot <- function (j) {
  ...
  writeRaster(r2080, filename=paste(w1,speclist[j],"_pred1km6_2080",sep=""), format="GTiff",overwrite=TRUE)
  r2050 <- raster::predict(bs2050, brt1, type="response", n.trees=brt1$n.trees)
  writeRaster(r2050, filename=paste(w1,speclist[j],"_pred1km6_2050",sep=""), format="GTiff",overwrite=TRUE)  
  
  rast <- raster(paste(w1,speclist[j],"_pred1km6.tif",sep=""))
  prev <- cellStats(rast, 'mean')	
  max <- 3*prev
  
  png(file=paste(w1,speclist[j],"_pred1km6_2080.png",sep=""), height=850, width=600)
  ...
  png(file=paste(w1,speclist[j],"_pred1km6_2050.png",sep=""), height=850, width=600)
  par(cex.main=1, mfcol=c(1,1), oma=c(0,0,0,0))
  par(mar=c(0,0,5,0))
...
cvstatsum <- function (speclist) {
  cvstats <- read.csv(paste(w1,speclist[1],"cvstats6.csv",sep=""))
  ...
  for (j in 2:length(speclist)) {
    x<-try(cv2 <- read.csv(paste(w1,speclist[j],"cvstats6.csv",sep="")))
...
for (j in 41:length(speclist)) {
  x<-try(rast <- raster(paste(w1,speclist[j],"_pred1km6.tif",sep="")))
  if(class(x)=="try-error"){
  ...
  dat1 <- right_join(specdat2001x,survey2001[,1:2],by=c("SS","PKEY")) 
  ...
  s2001 <- left_join(dat11,specoff, by=c("SPECIES","PKEY","SS"))
  ...
  dat2 <- inner_join(specdat2011x,survey2011[,1:2],by=c("SS","PKEY"))
  ...
  if (class(x1) != "NULL") {
    save(brt1,file=paste(w1,speclist[j],"brt6.R",sep=""))
    brtplot(j)
  }
...
for (j in 1:length(speclist)) {
  x1 <- try(load(paste(w1,speclist[j],"brt6.R",sep="")))
...
write.csv(cvstats,file=paste(w1,"_cvstats6.csv",sep=""))
