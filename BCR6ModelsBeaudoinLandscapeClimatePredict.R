...
surveyXY <- dplyr::left_join(survey,cdat[,1:3],by=c("SS"))
coordinates(surveyXY) <- c("X","Y")
surveyXY$prov <- cbind(surveyXY,raster::extract(prov,surveyXY))
...
dat_2011 <- cbind(dat2011,raster::extract(sampsum25,as.matrix(cbind(dat2011$X,dat2011$Y))))
...
dat_2001 <- cbind(dat2001,raster::extract(sampsum25,as.matrix(cbind(dat2001$X,dat2001$Y))))
...
prev <- function (speclist) {
  rast <- raster(paste(w,speclist[1],"_pred1km8.tif",sep=""))
  prev <- cellStats(rast, 'mean')	
  ...
  for (j in 2:length(speclist)){
    x1 <- try(rast<-raster(paste(w,speclist[j],"_pred1km8.tif",sep="")))
...
#   cvstatmean <- as.data.frame(cbind(as.character(cvstats[,1]),rowMeans(cvstats[,2:5])))
#   names(cvstatmean) <- c("stat",as.character(speclist[1]))
#   for (j in 2:length(speclist)) {
#     x<-try(cv2 <- read.csv(paste(w,speclist[j],"cvstats8.csv",sep="")))
#     if(class(x) != "try-error") {
#       cvstatmean <- as.data.frame(cbind(cvstatmean,rowMeans(cv2[,2:5])))
...
cvstats2 <- function (speclist) {
  x <- try(load(paste(w,speclist[1],"brt8.R",sep="")))
  varimp <- read.csv(paste(w,speclist[1],"varimp8.csv",sep=""))
  cvstats <- as.data.frame(brt1$cv.statistics[c(1,3)])
  cvstats$deviance.null <- brt1$self.statistics$mean.null
  cvstats$deviance.exp <- (cvstats$deviance.null-cvstats$deviance.mean)/cvstats$deviance.null
  cvstats$SPEC <- speclist[1]
  for (j in 2:length(speclist)) {
    x <- try(load(paste(w,speclist[j],"brt8.R",sep="")))
    if(class(x)!="try-error"){
      varimp <- read.csv(paste(w,speclist[j],"varimp8.csv",sep=""))
...
#   for (j in 2:length(speclist)) {
#     x<-try(varimp1 <- read.csv(paste(w,speclist[j],"varimp8.csv",sep="")))
#     if(class(x)!="try-error"){
...
write.csv(prevtab,file=paste(w,"_prevtab8.csv",sep=""))
...
# write.csv(statscombo,file=paste(w,"_statscombo8.csv",sep=""))
