...
brtplot <- function (j) {
  load(paste(w,speclist[j],"brt6.R",sep=""))
  varimp <- as.data.frame(brt1$contributions)
  write.csv(varimp,file=paste(w,speclist[j],"varim6.csv",sep=""))
...
mapplot <- function (j) {
  rast <- raster(paste(w,speclist[j],"_pred1km6.tif",sep=""))
  prev <- cellStats(rast, 'mean')	
  max <- 3*prev
  png(file=paste(w,speclist[j],"_pred1km6.png",sep=""), height=800, width=650)
  par(cex.main=1.2, mfcol=c(1,1), oma=c(0,0,0,0))
...
cvstatsum <- function (speclist) {
  cvstats <- read.csv(paste(w,speclist[1],"cvstats6.csv",sep=""))
  ...
  for (j in 2:length(speclist)) {
    x<-try(cv2 <- read.csv(paste(w,speclist[j],"cvstats6.csv",sep="")))
...
for (j in 1:length(speclist)) {
  ...
  datcombo <- na.omit(datcombo[,c(1:26,28:38,40:47,49:50,52:57)])
  
  potvar <- datcombo[,10:51]
  var <- get_cn(potvar)
  ...
  if (class(x1) != "NULL") {
    save(brt1,file=paste(w,speclist[j],"brt6.R",sep=""))
    brtplot(j)
  }
